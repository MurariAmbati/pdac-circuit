# PDAC Synthetic Gene-Circuit Pipeline, Results
...
>
> This file predates the review arc recorded in **[REVIEW_RESPONSE.md](REVIEW_RESPONSE.md)** (§1–§28)
> and the addenda **[docs/ADDENDUM_DYNAMICS.md](docs/ADDENDUM_DYNAMICS.md)**,
> **[docs/ADDENDUM_RAC_V2.md](docs/ADDENDUM_RAC_V2.md)**,
> **[docs/ADDENDUM_CHROMATIN.md](docs/ADDENDUM_CHROMATIN.md)**. In particular:
>
...
cross-domain transfer, and the honest PANC-1 negative are in [docs/ADDENDUM_DATA_SCALING.md](docs/ADDENDUM_DATA_SCALING.md).
...
4DN experiment set **4DNESCCP4KTY** (Hi-C on PANC-1), hg38, using the **released derived tracks** instead of the 2.6 GB contact matrix, exactly the protocol's "insulation scores ... A-Bin/B-Bin
...
- **Honest negative:** the in-vivo graph's apparent selective-dependency AUC (0.63–0.66) is **noise**, only **3** nodes pass `sel > 0.25`, and the value collapses to 0.43 at threshold 0.45. No claim made.
...
- **Leave-cell-line-out CV** ~~(the strong result)~~ **[DEMOTED, leaked whole-panel statistics; internal diagnostic only]**: refit on 53 lines, the held-out real cell state
  is a **2× lower-residual fixed point than a permuted null (0.064 vs 0.128, Wilcoxon p ≈ 0)**, the learned regulatory dynamics **generalise to unseen PDAC cell lines**.
...
**Multi-seed reproducibility** (`results/chromatin_ensemble.json`, `figures/fig_chromatin_training.png`), independent random-seed retrainings on the real shards:

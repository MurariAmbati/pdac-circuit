...
<figure id="fig8">
  ...
  <figcaption><b>Figure 8.</b> One designed circuit from the classical-subtype run. Panel a draws the delivered construct in SBOL Visual notation, where a bent arrow is a promoter, a pointed pentagon a coding sequence and a T-bar a terminator, so the effector and guide cassettes can be read the way a molecular biologist would read them. Panel b places the same parts at the target locus on chr18, with the dCas9-KRAB complex loaded on its guide RNA and a blunt head marking repression at the GATA6 promoter. Panel c is not a sketch. It is the ODE in <code>pdac_circuit.circuit.ode</code> integrated on this circuit's own rate constants, which reproduce the published knockdown of 0.647 exactly, and it shows the overshoot and settling that the negative feedback produces. Generated by <code>scripts/make_circuit_figure.py</code>.

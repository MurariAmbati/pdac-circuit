...
recorded in [REVIEW_RESPONSE.md]({{ '/reports/review/' | relative_url }}) §1/§15/§15b.
...
§6 proposed operating above the bifurcation. That is a hypothesis, and
`scripts/gain_sweep_rescue.py` → `results/gain_sweep_rescue.json` tests it directly: refit RAC at
gains 4–8 (each fit optimises W, b for its gain), and at each measure convergence, spectral radius,
and whether the collapse essentiality score beats the fixed degree reference (AUC 0.629, the §15
endpoint). Interpretation predeclared: a rescue requires a convergent/stable regime *and*
collapse > degree.
...
*Bistability axis.* Convergence is **0.00 at every gain**. Not one refit produces a map that
settles. Even where ρ < 1 (gains 7, 8), the map still does not converge — because with nothing
converging, ρ is measured at a 1000-iteration transient snapshot, not at a fixed point, so ρ < 1
there certifies nothing. The characterization's "bifurcation at 5.9" is hysteresis between two
non-converged transient means, not two stable basins. Raising the gain alone does **not** yield a
bistable system with this fit; §6(1)/(2) — enforcing true, stable fixed points — is the binding
requirement, not the gain.

*Prediction axis.* Collapse never robustly beats degree. Four of five gains are negative; the lone
positive (gain 7, +0.045) is one point in five, isolated (both neighbours negative), sits in a
non-convergent regime, and is well inside §15's ΔAUC noise band (95% CI ±0.11). It is reported, not
claimed — treating it as a rescue would be selecting one gain out of five on the outcome, the exact
error §15 was built to avoid.

**Conclusion — the §15 retraction is structural, not a tuning artifact.** Dynamics can only
propagate what the graph encodes; if essentiality is not in the co-expression graph beyond degree
(and §15/§15b showed it is not — partial ρ = 0.028, KRAS at the 8th percentile), then no
reparameterisation of the dynamics over that graph can manufacture it. The failure is in the
substrate, not the operating point. This closes the constructive question: RAC as posed is not one
hyperparameter away from working, and a genuine rebuild would have to change what the graph encodes
(e.g. directed/causal edges, perturbation data), not merely how the dynamics run over it.
